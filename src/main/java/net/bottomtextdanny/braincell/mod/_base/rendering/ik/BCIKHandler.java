...
//    }
//
//    public Vector3f getLocalPos(T object, int index) {
//
//        float xDelta = 0, yDelta = 0, zDelta = 0, xRot = 0, yRot = 0;
...
//    public Vector3f getLocalTopPos(T object, int index) {
//        float xDelta = 0, yDelta = 0, zDelta = 0, xRot = 0, yRot = 0;
//        for (int i = index; i < this.joints.length; i++) {
//            IKPartData data = this.joints[i].dataGetter().apply(object);
//            if (i + 1 < this.joints.length) {
//                BCJoint joint = this.joints[i + 1].joint();
//                xRot = data.xRot;
        ...
        for (int j = 0; j < 1; j++) {
            ...
            for (int i = this.joints.length - 1; i >= 0; i--) {
                ...
                if (i + 1 < this.joints.length) {
                    ...
                    dragJoint(i, object, data, current.getConstraint(), childData.x, childData.y, childData.z, child.joint().x, child.joint().y, child.joint().z);
                } else {
                    dragJoint(i, object, data, current.getConstraint(), xGoal, yGoal, zGoal, 0, 0, 0);
                }
            ...
            for (int i = 0; i < this.joints.length; i++) {
                ...
                if (i != this.joints.length - 1) {
                    BCIKPart<T> child = this.joints[i + 1];
                    IKPartData childData = child.dataGetter().apply(object);
                    dragJoint(i, object, data, part.getConstraint(), childData.x, childData.y, childData.z, child.joint().x, child.joint().y, child.joint().z);
                }
