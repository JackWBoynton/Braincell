...
public class HandRendering {
    ...
    private static void hurtCameraEffect(PoseStack matrixStackIn, float partialTicks) {
        ...
        if (mc.getCameraEntity() instanceof LivingEntity) {
            ...
            f = BCMath.sin(f * f * f * f * (float)Math.PI);
    ...
    private static void aBob(PoseStack matrixStackIn, float partialTicks) {
        ...
        if (mc.getCameraEntity() instanceof Player) {
            ...
            matrixStackIn.translate(BCMath.sin(f1 * (float)Math.PI) * f2 * 0.5F, -Math.abs(BCMath.cos(f1 * (float)Math.PI) * f2), 0.0D);
            matrixStackIn.mulPose(Vector3f.ZP.rotationDegrees(BCMath.sin(f1 * (float)Math.PI) * f2 * 3.0F));
            matrixStackIn.mulPose(Vector3f.XP.rotationDegrees(Math.abs(BCMath.cos(f1 * (float)Math.PI - 0.2F) * f2) * 5.0F));
        }
    }
}
