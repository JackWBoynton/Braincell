...
import bottomtextdanny.braincell.libraries.particle.local_sprites.SpriteGroup;
...
public abstract class BCParticle extends TextureSheetParticle {
    ...
    public void tick() {
        ...
        if (!isCompressed()) {
	        if (this.age < this.lifetime) {
                int localAge = this.age % ticksForEachFrameDefault.length;

                if (ticksForEachFrame[localAge]-- <= 0) {
                    if (localAge == ticksForEachFrame.length - 1)
                        resetFrames();
                    this.age++;

                    if (this.age < this.lifetime) {
                        setLocalSprite(this.age % ticksForEachFrameDefault.length);
                    }
                }
	        } else {
		        remove();
    ...
    public void resetFrames() {
        this.ticksForEachFrame = this.ticksForEachFrameDefault.clone();
    }
    ...
    public void setSimpleFrameTicks(int spriteTicks) {
        ...
        this.maxTime = lifetime * spriteTicks;
