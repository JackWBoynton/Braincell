...
import bottomtextdanny.braincell.libraries.psyche.targeting.*;
...
public class MoveReactionToEntityAction<T extends PathfinderMob, U extends Entity> extends Action<T> {
    ...
    public boolean canStart() {
        ...
        if (this.active() && !mob.isVehicle()
                && focus != null) {
            BlockPos goalPos = this.posFinder.compute(mob.blockPosition(), mob, UNSAFE_RANDOM, focus);

            if (goalPos == null) return false;

            this.goal = mob.getNavigation().createPath(goalPos, 0);
    ...
    public boolean shouldKeepGoing() {
        T mob = this.mob;
        return this.active() && !mob.isVehicle() &&
                (this.goal != null && mob.getNavigation().isInProgress())
    ...
    protected void updateFocus() {
        ...
        this.focus = this.searchPredicate.search(this.mob,
                ...
                RangeTest.awayFrom(this.mob, range, Distance3s.d3Manhat()),
