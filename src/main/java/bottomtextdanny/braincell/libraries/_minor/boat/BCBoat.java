...
public class BCBoat extends Boat implements BCDataManagerProvider, EntityClientMessenger {
	...
	private final Persistent<Item> boatItem;
	...
	public BCBoat(EntityType<? extends Boat> type, Level world) {
		...
		this.boatItem = bcDataManager().addSyncedData(Persistent.of(DATA_REF));
	}
	...
	public void updateClientTexture() {
		if (!this.level.isClientSide()) {
			sendClientMsg(UPDATE_TEXTURE_CALL, PacketDistributor.TRACKING_ENTITY.with(() -> this), PacketData.of(BuiltinSerializers.INTEGER, Item.getId(this.boatItem.get())));
		}
	}
