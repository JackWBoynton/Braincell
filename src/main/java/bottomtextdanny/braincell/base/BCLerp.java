...
public final class BCLerp {
   ...
   public static Vec2 get(float approachValue, Vec2 start, Vec2 end) {
      return new Vec2(get(approachValue, start.x, end.x), get(approachValue, start.y, end.y));
   }

   public static Vec3 get(double approachValue, Position start, Position end) {
      return new Vec3(get(approachValue, start.x(), end.x()), get(approachValue, start.y(), end.y()), get(approachValue, start.z(), end.z()));
   }

   public static Vector3f get(float approachValue, Vector3f start, Vector3f end) {
      return new Vector3f(get(approachValue, start.x(), end.x()), get(approachValue, start.y(), end.y()), get(approachValue, start.z(), end.z()));
   }
}
